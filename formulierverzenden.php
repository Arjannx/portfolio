<?php
$naar = 'arjandeconinck@live.nl'; // Waar moet het naartoe?
$onderwerp = 'Contactformulier http://arjandeconinck.tk'; // Het onderwerp van het bericht
...
if(isset($_POST['versturen'])) // Als het formulier verzonden is door op de verzend knop te klikken
{
  ...
  {
    print '<p>Helaas, het voornaam veld is verplicht maar is nu niet ingevuld!</p><br />';
    echo "<a href='contact.php'>Ga hier terug naar het contactformulier</a>";
  ...
  {
    print '<p>Helaas, het achternaam veld is verplicht maar is nu niet ingevuld!</p><br />';
    echo "<a href='contact.php'>Ga hier terug naar het contactformulier</a>";
    $fout = true;
  }
  if(empty($email)) // Als het email veld niet is ingevuld
  {
    print '<p>Helaas, het email veld is verplicht maar is nu niet ingevuld!</p><br />';
    echo "<a href='contact.php'>Ga hier terug naar het contactformulier</a>";
    $fout = true;
  }
  if(!filter_var($email, FILTER_VALIDATE_EMAIL)) // Als het email adres niet correct is
  {
    print '<p>Helaas, het email adres is niet correct!</p><br />';
    echo "<a href='contact.php'>Ga hier terug naar het contactformulier</a>";
    $fout = true;
  }
  if(empty($bericht)) // Als het bericht veld niet is ingevuld
  {
    print '<p>Helaas, het bericht veld is verplicht maar is nu niet ingvuld!</p><br />';
    echo "<a href='contact.php'>Ga hier terug naar het contactformulier</a>";
    $fout = true;
  }
  ...
  if($fout == false) // Als er niks fout is (alles is dus netjes ingevuld)
  {
    ...
    {
      print '<p>Het bericht is succesvol verzonden!</p><br />';
      echo "<a href='contact.php'>Ga hier terug naar het contactformulier</a>";
    }
    else
    {
      print '<p>Helaas, er is wat fout gegaan tijdens het verzenden van het formulier.</p><br />';
      echo "<a href='contact.php'>Ga hier terug naar het contactformulier</a>";
    }
  }
}
